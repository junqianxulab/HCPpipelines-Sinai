...
if [[ "$HCPver" != "v3.22.0-beta.2-Sinai" ]] ; then
	echo "ERROR: HCP Pipeline version must = v3.22.0-beta.2-Sinai"
...
if [[ "$WBver" != "Version: 1.2.3" ]] ; then
        echo "ERROR: Connectome Workbench version must = 1.2.3"
...
if [ "$FSLver" != "5.0.6" ] ; then
	echo "ERROR: FSL version must = 5.0.6"
	exit 41
fi
if [ $# -eq 0 ] ; then
...
echo -e "\n\nWARNING: home directory currently set to /sc/orga/projects/xuj09a/gabbay_storage/Subjects/test\n\n"
cd ${home}/${ID}
...
if [ -n "$LR" ] && [ -z "$AP"] ; then
	...
	echo -e "\nERROR: Fieldmaps must be a single paired LR/RL or AP/PA set. Check input options."
	exit 66
fi
# Parse other inputs
...
echo -e "\n\nRunning PreFreeSurfer pipeline.\n\n"
...
if [ "$?" = "0" ] ; then
	echo -e "\n\nPreFreeSurfer pipeline complete. Phew!\n\nRunning FreeSurfer pipeline.\n\n"
	cd ${home}/${ID}
...
#		--seed=true # optionally turn on random seed generation for recon-all, I believe this can help if FS segmentation is performing poorly
	...
	echo -e "\n\nERROR: PreFreeSurfer pipeline did not complete normally; check files and re-run."
	exit 42
fi

if [ "$?" = "0" ] ; then
	echo -e "\n\nFreeSurfer pipeline complete! Check results before running PostFreeSurfer pipeline."
...
#		--refmyelinmaps=${HCPPIPEDIR_Templates}/standard_mesh_atlases/Conte69.MyelinMap_BC.164k_fs_LR.dscalar.nii
	...
	echo -e "\n\nERROR: FreeSurfer pipeline did not complete normally; check files and re-run."
	exit 43
fi
